<h1 align="center">DAJ's portfolio v1</h1>

![Mockup](https://ntdqhebsazcdgncewudr.supabase.co/storage/v1/object/public/daj/mockup.png)

[![forthebadge](data:image/svg+xml;base64,)](https://forthebadge.com)

## Description

---

This open-source project serves as my personal portfolio which will feature my resume, projects and everything an employer needs to know about me.
<br>
<br>
Feel free to fork and modify the project. Component library is available in `src/components` directory.

## Techonoly stack

---

**Frontend**

- React.js
- TailwindCSS

**Backend**

- Supabase

## Todo

---

- Create custom CMS for the blog and portfolio
